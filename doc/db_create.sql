...
-- Tue 21 Nov 2017 11:52:05 AM EST
    ...
    ELSE
        ...
        IF ( SELECT NOT EXISTS (SELECT 1  
			...
			WHERE Driver.idDriver IN (
					...
					BusinessDriver.HireStatus = 'hired' AND
                    Driver.Enrolled = 1)
			) ) 
		THEN
...
-- -----------------------------------------------------
-- procedure get_driver_from_phone
-- -----------------------------------------------------

DELIMITER $$
USE `dispatcher`$$
CREATE PROCEDURE `get_driver_from_phone` (
	IN p_phone CHAR(15)
)
BEGIN
	DECLARE ID INT;
...
BEGIN
	...
	WHERE b.idBusiness = p_idBusiness AND
		d.Enrolled = 1;
END$$
...
-- -----------------------------------------------------
-- procedure get_business_driver
-- -----------------------------------------------------

DELIMITER $$
USE `dispatcher`$$
CREATE PROCEDURE `get_business_driver` (
	IN p_idBusiness INT,
    IN p_idDriver INT
)
BEGIN
...
-- -----------------------------------------------------
-- procedure get_assoc_drivers_with_status
-- -----------------------------------------------------

DELIMITER $$
USE `dispatcher`$$
CREATE PROCEDURE `get_assoc_drivers_with_status` (
	IN p_idBusiness INT,
    IN p_Status ENUM('pending', 'hired', 'blocked')
)
BEGIN
...
-- -----------------------------------------------------
-- procedure get_business_from_url
-- -----------------------------------------------------

DELIMITER $$
USE `dispatcher`$$
CREATE PROCEDURE `get_business_from_url` (
	IN p_url CHAR(10)
)
BEGIN
	SELECT * FROM Business WHERE BusinessURL = p_url;
END$$
