...
CREATE TABLE IF NOT EXISTS `dispatcher`.`Business` (
  ...
  `MerchantID` VARCHAR(26) NOT NULL COMMENT 'Clover-given ID of the merchant',
...
CREATE TABLE IF NOT EXISTS `dispatcher`.`JobDriverURL` (
  ...
  `fk_idJobDriver` INT UNSIGNED NULL,
  PRIMARY KEY (`idJobDriverURL`),
  UNIQUE INDEX `URL_UNIQUE` (`URL` ASC),
  INDEX `fk_idJobDriver_JobDriverURL_idx` (`fk_idJobDriver` ASC),
  CONSTRAINT `fk_idJobDriver_JobDriverURL`
    FOREIGN KEY (`fk_idJobDriver`)
    REFERENCES `dispatcher`.`JobDriver` (`idJobDriver`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
BEGIN
	IF ( SELECT EXISTS (SELECT 1 FROM Driver WHERE PhoneNumber = p_phone) ) THEN
...
BEGIN
	IF ( SELECT NOT EXISTS (SELECT 1 FROM Driver WHERE idDriver = p_idDriver) ) THEN
...
BEGIN
	IF ( SELECT NOT EXISTS (SELECT 1 FROM DriverConfirmCode WHERE idDriverConfirmCode = p_id AND ConfirmCode = p_code) ) THEN
